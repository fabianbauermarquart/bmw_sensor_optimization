...
def main():
    ...
    def sum_price(choice_list: List[int]) -> float:
        """
        Evaluate price of a given sensor configuration.
        :param choice_list: list of 0s and 1s, where 0 => not included, 1 => included.
        :return: total price.
        """
        choice_vector = np.array(choice_list, dtype=float)
    ...
    def evaluate_choice_coverage(choice_list: List[int]) -> float:
        ...
        """
        coverage_grids_choice = np.array([coverage_grids[i]
